...
DOM 事件流中实际的目标在捕获阶段不会接收到事件，处理目标阶段在事件处理中被看成冒泡阶段一部分。“DOM2 级事件” 规范要求捕获阶段不会涉及事件目标，但实现 DOM2 级事件的浏览器都会在捕获阶段触发事件对象上的事件。结果，就是有两个机会在目标对象上操作事件。*在目标阶段的事件会触发该元素上（即事件目标）所有监听器，而不关心监听器注册在捕获阶段还是冒泡阶段，按添加顺序触发*
